...
\caption{Peak pump radiance \SI{4430}{\radiance}\\TA\_fourier measurements:\\ 6645-6653}
\end{subfigure}
...
\end{figure}
Usually photobleaching would be modelled as an exponential decay as a function of time exposed. The measurements above could not be fit sufficiently well over all delay times, even when using a moving mean to smooth out the irregularities slightly. Fitting was attempted with the raw data, not with fitted values shown in fig. \ref{fig:degradationfit}.



\begin{figure}[hbtp]
\centering
\includegraphics[width=\linewidth]{images/DegradationAtDelays.png}
\caption{Sample response over measurement time from fit results\\
The measurements have been slightly overfitted to more accurately represent the measurements shape, without a strict physical reasoning. As the usual averaging over more than one decay scan is missing, the data is a bit more noisy as can be seen in fig. \ref{fig:rawDegradation}.\label{fig:degradationfit}}
\end{figure}
