...
Furthermore the depth of focus may be visualized by plotting the beam waist diameter over distance for several relevant focal lengths in fig. ....
...
The factor is calculated from the Gaussian intensity distributions as seen in eq. \ref{CorrFactorGaussians}, of which the derivation is shown in REF back or in Bromiley, which was used to check correctness.\cite{Bromiley2014} The enumerator is the integral over essentially the probe intensity with the constant factor of the maximum pump intensity 
\begin{equation*}
...
\section{Produkt of two Gaussians}
...
& = S_n \cdot \frac{1}{\sqrt[2]{2\pi}\sigma_{new}} \cdot exp\left(-\frac{\left(x-\mu_{new}\right)^2}{2\sigma_{new}^2}\right)
\end{split}
\end{gather}
