...
Loads a \textcolor{blue}{JSON characterising the background file}, including a set offset of ROI, the path for the beam profile to be loaded and a width for fitting.
...
Correction Factor                           & $\sqrt{2}$ & $\approx1.05$ & $\approx 1.12$ & $\approx 2.24$ & $\approx 1.82$ & $\approx 5.66$        
\end{tabular}
\end{table}
...
\newline
For all measurements included here version 1.2 was used. The  ALGLIB software package is used and \textbf{(version number)}\footnote{ALGLIB for C\# licensed as per GPL 2} for fitting a Gaussian to the beamshape. Scottplot 4 is used to display the fitted data.\\
...
Zheng measurement is isotropic (anisotropy corrected considering both parallel and orthogonal pump probe polarization), while ours is parallel only.}
\end{figure}
...
As mentioned above irregularities over the area of the sample probed were attempted to linearly compensate for with a reference map at a pump 653 nm probe 680 nm combination. The attempt of a linear compensation does make sense if the variation is of sample thickness but not necessarily for density variations, see \ref{deriv:RadianceResponse}. In the case of drastic density variation features that are not of the plain coulombic monomer spectrum could be amplified and the states measured be quenched more quickly than for the monomer. Comparing the 1wt\% and 4wt\% graphs of the CW absorption it seems likely that a sample density variation of 100\% would be a minor influence on the transient absorption spectrum recorded, considering other uncertainties involved.
...
On the right probe power is varied at a pump power density of 4430 $\mathrm{\frac{W}{m^2}}$.\\
In red the degradation compensated points are shown, only accounting for the pump peak radiance. For the right subplot the history of the measurement is left, right and center. The relatively high probe intensity in the GSD band of the squaraine is likely to be the reason for the error in degradation compensation, since the highest radiance is the second measurement point. \label{fig:PowerVarRaw}}
\end{figure}
