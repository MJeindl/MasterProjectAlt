...
\textbf{
Tell them about what happens if probe intensity is too high. -> self GSD of probe that leads to bad measurement values}
...
\centering
\includegraphics[width = 0.5\linewidth]{images/ArtrayExamplePics/ArtrayControlledMain.PNG}
\caption{Artray Controlled window}
\end{figure}

\begin{figure}[hbtp]\label{fig:ArtrayFitting}
\centering
\includegraphics[width = 0.5\linewidth]{images/ArtrayExamplePics/FittingForm.PNG}
\caption{Fitting form artray}
\end{figure}
...
\item Misc: is where custom settings for output parameters as well as exposure can be set. Handing over a "µm per px" value and adjusting the power correctly allows a direct corrected readout of peak radiance and other fit parameters.\\The three exposure values can be quickly jumped to from a drop down menu in the main window and are supposed to help streamline the measurement process.
\end{itemize}
...
\item Reference: sends a reference image, behavior is like with save regarding livestreams, to the "LiveFit" window. There a fit of the reference is shown as a red graph, which when not set is flat.
\item Open: works like reference, however it opens the reference snapshot from a raw txt file of choice via file dialog.
\item Background: sets a background to subtract from the images for live fitting. Setting background does not alter the behavior of Save. Saved files are still plain raw unaltered data from the sensor. The background file location is automatically loaded within a session and is saved within the \textbf{windows filesystem? don't remember the name of the feature}
\end{itemize}
\subsubsection{Inner workings of the software}
The software package is based on the SDK provided freely by Artray in C\#, specifically the code example \textbf{"find name"}. 
...
For measurements to be valid there should be a linear correspondence of pump power and the measured absorbance. To test this multiple pump and probe power combinations were tested, at the same sample position with the same pump probe overlap. The linearity of the sample is shown for powers up to \textbf{use radiance instead} 10 µW pump and.... probe combinations. \textbf{Show the relatively linear plot}
